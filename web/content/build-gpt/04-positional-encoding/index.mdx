...
<Description>
  At first glance, this seems problematic: if we only feed word embeddings, the compatibility between "Alice" and "Bob" depends on *what* they are, not *where* they appear. Both sentences would produce identical scores. Nothing in these vectors explicitly encodes order, so it's not obvious how the model would distinguish Subject from Object. This motivates us to think about ways to inject position information, and what follows is one line of reasoning that leads to the original Transformer's approach.
</Description>
</div>
...
<Step title="3. What Might Make a Good Positional Encoding?">
  <Description>
    We've tried two simple approaches and saw why they feel awkward. Before we try again, let's step back and think about what properties we *might* want in a positional encoding. These aren't proven requirements, just reasonable hypotheses about what could make the signal useful.
  </Description>
  ...
  <Description>
    Ideally, we'd want the model to handle sequences longer than what it saw during training. An encoding that extends naturally to larger positions *seems* appealing. (In practice, this intuition didn't fully pan out. Later research showed models still struggle with lengths far beyond training, regardless of encoding scheme. But it was part of the original reasoning, so we include it here.)
  </Description>
...
<Step title="4. Borrowing from Number Systems">
  <Description>
    With those properties in mind, let's look for inspiration. We already use systems that satisfy most of them every day. Consider how we write the number **459**: each digit is **bounded** (0 to 9), the full pattern is **unique** (no other number looks like 459), and moving by +1 follows a **consistent pattern** (459 → 460 → 461).
  </Description>
...
Decimal has the multi-column idea and handles infinite positions, but it is not a great positional signal if we try to inject it directly into embeddings. First, **scale:** digits 0 to 9 are not huge, but if we literally inject them as-is, they can still shift the embedding distribution, which is often kept roughly zero-centered. Second, **discontinuities:** decimal digits are discrete. When the ones digit wraps from 9 to 0, the representation changes abruptly. That makes the signal less structured for learning local relationships.  
  ...
  <Description>
    This smooth oscillation preserves the core insight from our number system analogy: multiple dimensions changing at different speeds, all staying in a bounded range, giving each position a distinctive signature. Unlike binary, the values change gradually from one position to the next. All that remains is choosing the function that produces this kind of smooth cycle.
  </Description>
</Step>
...
<Step title="7. Building the Formula">
  ...
  <Description>
    How slow should the slowest frequency be? Think back to number systems: a 2-digit counter wraps from 99 back to 00, making position 100 look identical to position 0. The same happens if our slowest wave completes a full cycle within the context window. To avoid this, we want the slowest wavelength to exceed typical context lengths. With a wavelength of 10,000 × 2π ≈ 62,800 positions, contexts of 2,048 or even 8,192 tokens stay within the first quarter of the cycle (0 to π/2), where sine only increases.
  </Description>

  <Description>
    So we want frequencies ranging from 1 (fastest) down to 1/10000 (slowest), spread across our d_model/2 slots. We can spread them evenly on a logarithmic scale using:
  </Description>
  ...
  <Description>
    The exponent `2i/d_model` normalizes the frequency index to a 0→1 range. Since i goes from 0 to d_model/2, we multiply by 2 to span the full range. At `i = 0`, the exponent is 0, so ω = 1 (fastest). At `i = d_model/2`, the exponent reaches 1, so ω = 1/10000 (slowest).
  </Description>
...
<Step title="8. Summary">
  <Callout type="success" title="What We Learned">
    * **Raw position indices** have unbounded magnitude that can overwhelm embeddings
    * **Number systems** offer inspiration: multiple columns cycling at different speeds
    * **Sinusoidal waves** smooth discrete cycling into continuous, bounded values
    * **Sine/cosine pairs** make position shifts a simple linear operation
    * **Frequency spread** from fast to slow distinguishes both nearby and distant positions
    * **Addition** combines position and meaning without increasing dimensionality
  </Callout>
