...
<Step title="7. Building the Formula">
  ...
  <Description>
    The formula ω_i = 1/base^(2i/d_model) achieves this, with the exponent `2i/d_model` sliding from 0 at the first pair (giving ω = 1) toward 1 at the last pair (giving ω ≈ 1/base), placing all intermediate frequencies on a smooth geometric curve. The original Transformer uses base = 10000, which yields a slowest wavelength of about 63,000 positions, well beyond typical context windows at the time. Any base large enough to cover your target sequence length works. The value 10000 was simply a safe default.
  </Description>
