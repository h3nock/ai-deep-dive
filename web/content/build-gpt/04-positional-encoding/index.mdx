...
<Step title="6. Making Relative Positions Easy">
  ...
  <Description>
    The rotation matrix depends **only on k** (the offset), not on the starting position. Whether you're at position 5, 50, or 500, shifting by k applies the same transformation. This means "k steps ahead" looks the same from anywhere in the sequence, which could help the model learn position-independent attention patterns.
  </Description>
</Step>
