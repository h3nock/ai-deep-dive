...
<Callout type="success" title="Summary">
  ...
  * By splitting the model dimension across heads (`d_k = d_model/h`), the layer maintains the same computational cost as a single-head mechanism
</Callout>

<Description>
  Each token has now integrated information from the entire input sequence. The next chapter introduces the **feed-forward network**, which processes these enriched vectors independently.
</Description>
