...
<Step title="How It Works">
  <Description>
    Now that we understand the goal of capturing multiple independent relationships, let's walk through how the model achieves this.
  </Description>

  <Description>
    We start by projecting the input `X` into distinct Query, Key, and Value matrices for each head using independent weights (`W_Q^i`, `W_K^i`, `W_V^i`). Since each head has its own unique projection, it maps the input into a different subspace. This lets each head define what counts as relevant in its own way.
  </Description>
  ...
  <Description>
    Finally, the independent streams are merged. Concatenating the outputs from all heads restores the full dimensionality, yet keeps the information from each head isolated in its own segment. The projection through `W_O` mixes the distinct features discovered by different heads into a single, unified representation.
  </Description>
