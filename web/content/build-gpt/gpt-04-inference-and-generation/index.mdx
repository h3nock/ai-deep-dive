...
```
  ...
  <Description>
    Turning that base model into a chatbot involves post-training, starting with supervised fine-tuning (SFT), which reuses the same next-token training on instruction-and-response data, often followed by preference tuning such as RLHF. To explore that pipeline, see Karpathy's [nanochat](https://github.com/karpathy/nanochat).
  </Description>
</Step>
