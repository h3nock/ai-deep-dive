...
```
  ...
  <Callout type="note" title="Lighter Config">
...
<div className="my-6 p-5 bg-terminal rounded-lg border border-border overflow-x-auto">
  <div className="inline-flex flex-col gap-4 font-mono text-sm">
    ...
    <div className="flex flex-col gap-2">
      <div className="text-xs text-subtle uppercase tracking-wider">At each position, predict the next token</div>
      {[
        { ctx: [11], target: 318 },
...
```
  ...
  <Description>
    The data pipeline is complete. In the next chapter, we build the model
    that trains on these batches.
  </Description>
</Step>
