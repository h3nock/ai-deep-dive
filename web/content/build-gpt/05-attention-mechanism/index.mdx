          ...
          {[
            ...
            <div key={i} className="flex">
              {row.map((score, j) => (
                <div
                  key={j}
                  className="w-14 h-12 flex items-center justify-center text-sm border-r border-b border-zinc-800 last:border-r-0"
                  style={{ backgroundColor: `rgba(16, 185, 129, ${score * 0.6})` }}
                >
                  <span className={score > 0.5 ? 'text-white' : 'text-zinc-400'}>{score.toFixed(1)}</span>
                </div>
              ))}
            </div>
          ))}
