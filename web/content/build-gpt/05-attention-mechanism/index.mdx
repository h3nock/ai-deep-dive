...
<Step title="Measuring Relevance">
  ...
  <div className="my-4 p-4 bg-terminal rounded-lg border border-border font-mono text-sm text-center">
  ...
  <div className="my-6 flex justify-center overflow-x-auto">
    <div className="inline-block">
      <div className="flex">
        ...
        <div className="text-xs text-subtle mb-2 text-center" style={{ width: '224px' }}>Key</div>
      </div>
      <div className="flex">
        <div className="w-16"></div>
        <div className="w-14"></div>
        <div className="flex">
          {['The', 'bright', 'student', 'reads'].map((token, i) => (
            <div key={i} className="w-14 h-6 flex items-center justify-center text-xs text-subtle font-mono">{token}</div>
          ))}
        </div>
      </div>
      <div className="flex">
        <div className="flex flex-col justify-center w-16">
          <div className="text-xs text-subtle text-right pr-2">Query</div>
        </div>
        <div className="flex flex-col">
          {['The', 'bright', 'student', 'reads'].map((token, i) => (
            <div key={i} className="w-14 h-12 flex items-center justify-end pr-2 text-xs text-subtle font-mono">{token}</div>
          ))}
        </div>
        <div className="border border-border-hover rounded overflow-hidden">
          {[
            [0.2, 0.1, 0.3, 0.1],
            [0.1, 0.2, 0.8, 0.4],
            [0.2, 0.7, 0.3, 0.1],
            ...
            <div key={i} className="flex">
              {row.map((score, j) => (
                <div
                  key={j}
                  className="w-14 h-12 flex items-center justify-center text-sm border-r border-b border-border last:border-r-0"
                  style={{ backgroundColor: `color-mix(in srgb, var(--color-accent-1) ${score * 60}%, transparent)` }}
                >
                  <span className={score > 0.5 ? 'text-primary' : 'text-secondary'}>{score.toFixed(1)}</span>
                </div>
              ))}
            </div>
          ))}
      ...
      <div className="flex">
        <div className="w-16"></div>
        <div className="w-14"></div>
        <div className="text-xs text-subtle mt-2 text-center" style={{ width: '224px' }}>Higher scores = stronger relevance</div>
      </div>
    </div>
  </div>
</Step>
...
<Step title="Combining Values">
  ...
  <Description>
    As the dimension of the Query and Key vectors grows, dot products tend to produce larger numbers because each additional dimension adds to the sum. When softmax receives these larger values, it concentrates nearly all weight on whichever token has the highest score. The output becomes essentially one token's Value, losing the ability to blend information from multiple sources. Scaling the scores down by √d before softmax keeps the values moderate, allowing weight to spread across several tokens when useful.
  </Description>
...
<Step title="Causal Masking">
  <Description>
    As we saw in the introduction, the model learns by predicting the next word. Given "The cat sat on the", it predicts "mat" and adjusts its weights based on the error. During training, however, we feed the model entire sequences at once for efficiency. If position 5 can attend to position 6, it could simply copy the answer instead of learning to predict it. This is information leakage.
  </Description>
  ...
  <div className="my-6 flex justify-center overflow-x-auto">
    <div className="inline-block">
      <div className="flex">
        ...
        <div className="text-xs text-subtle mb-2 text-center" style={{ width: '200px' }}>Key Position (attending to)</div>
      </div>
      <div className="flex">
        <div className="w-20"></div>
        <div className="w-6"></div>
        <div className="flex">
          {['1', '2', '3', '4', '5'].map((k, i) => (
            <div key={i} className="w-10 h-6 flex items-center justify-center text-xs text-subtle">{k}</div>
          ))}
        </div>
      </div>
      <div className="flex">
        <div className="flex flex-col justify-center w-20">
          <div className="text-xs text-subtle text-right pr-3">Query Position<br/><span className="text-[10px]">(attending from)</span></div>
        </div>
        <div className="flex flex-col">
          {['1', '2', '3', '4', '5'].map((q, i) => (
            <div key={i} className="w-6 h-10 flex items-center justify-center text-xs text-subtle">{q}</div>
          ))}
        </div>
        <div className="border border-border-hover rounded">
          {[1, 2, 3, 4, 5].map((q) => (
            <div key={q} className="flex">
              {[1, 2, 3, 4, 5].map((k) => (
                <div
                  key={k}
                  className={`w-10 h-10 flex items-center justify-center text-sm border-r border-b border-border last:border-r-0 ${
                    k <= q ? 'bg-accent-1/20 text-accent-1' : 'bg-surface/50 text-muted'
                  }`}
                >
                  {k <= q ? '✓' : '✗'}
                </div>
      ...
      <div className="flex">
        ...
        <div className="flex justify-center gap-6 text-xs mt-3" style={{ width: '200px' }}>
          <div className="flex items-center gap-1.5">
            <div className="w-3 h-3 bg-accent-1/20 border border-accent-1/30 rounded"></div>
            <span className="text-subtle">Can attend</span>
          </div>
          <div className="flex items-center gap-1.5">
            <div className="w-3 h-3 bg-surface/50 border border-border-hover rounded"></div>
            <span className="text-subtle">Masked (−∞)</span>
          </div>
        </div>
      </div>
    </div>
