...
<Step title="Concrete Example">
  <Description>
    Let's see how the string **"Hi 👋"** is converted into bytes using UTF-8.
  </Description>
  ...
  <div className="flex flex-col gap-6 my-6">
    ...
    <div>
      ...
      <div className="text-lg leading-8 text-slate-700 dark:text-slate-300 mb-6">
...
<Step title="4. Training on Raw Bytes">
  <Description>
    ...
    <Callout type="note" title="Key Questions">
  ...
  <ThinkingProcess 
    ...
    hint={
      <div>
        ...
        <br/><br/>
        Imagine that instead of bytes, we assigned a unique ID to every unique word (e.g., "Apple" = 502, "Zebra" = 804, "ሰላም "= 90,210).
  ...
  >
    ...
    <Description>
      This approach is actually very logical. In fact, if you tried this, it *would* work—the model would learn. But as we scale up, we run into specific engineering trade-offs.
    </Description>
    ...
    <div className="flex flex-col gap-6 my-6">
      ...
      <div className="p-6 rounded-xl bg-green-50 dark:bg-green-900/10 border border-green-100 dark:border-green-800/30">
        ...
        <ul className="space-y-4 text-sm text-green-900 dark:text-green-100">
          <li>
            <strong>It can read anything:</strong> Since every digital file is made of bytes, this model could read English, Chinese, Python code, or even image files without ever crashing. It effectively solves the "Unknown Word" problem forever.
          </li>
          <li>
            <strong>Tiny Memory:</strong> The model only needs to learn 256 possible input values. This makes the input layer incredibly small and efficient.
          </li>
        </ul>
      </div>
      ...
      <div className="p-6 rounded-xl bg-red-50 dark:bg-red-900/10 border border-red-100 dark:border-red-800/30">
        ...
        <ul className="space-y-4 text-sm text-red-900 dark:text-red-100">
          <li>
            <strong>It is too "zoomed in":</strong>
            <ul className="list-disc pl-4 mt-2 space-y-1 opacity-90">
              <li>To a human, <strong>"Apple"</strong> is 1 unit of meaning.</li>
              <li>To this model, it is 5 separate steps: <code>65</code>, <code>112</code>, <code>112</code>, <code>108</code>, <code>101</code>.</li>
              <li>The model has to waste massive amounts of compute power just to learn how to spell words letter-by-letter.</li>
            </ul>
          </li>
          <li>
            <strong>The Memory Limit:</strong> Neural networks have a fixed "context window".
            <ul className="list-disc pl-4 mt-2 space-y-1 opacity-90">
              <li>A sentence like <em>"The quick brown fox"</em> is <strong>4 words</strong>.</li>
              <li>But in bytes, it is <strong>19 numbers</strong>.</li>
              <li>If we use bytes, we fill up the model's memory ~5x faster.</li>
            </ul>
          </li>
        </ul>
      </div>
    ...
    <Description>
      * **Words** are too specific (there are too many of them).
      * **Bytes** are too small (the sequences get too long).
