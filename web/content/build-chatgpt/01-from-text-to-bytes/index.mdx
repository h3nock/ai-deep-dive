...
<Step title="2. Encoding">
  ...
  <Callout type="note" muted>
    Hexadecimal is a human‑friendly way to write binary numbers. Unicode code points are shown in hex (`U+XXXX`), and we’ll also show encoded byte streams in hex. The reason is: 1 hex digit represents 4 bits, so 2 hex digits represent exactly 1 byte. That makes long byte sequences compact, byte‑aligned, and easy to scan or debug.
  </Callout>
