...
<Step title="1. Text Representation">
  ...
  <Callout type="note" muted>
    Before Unicode, every region invented its own encoding. Americans used ASCII, Russians used KOI8, Japanese used Shift-JIS. A file written in one encoding looked like garbage when opened with another. Unicode ended this chaos.
  </Callout>
...
<Step title="2. Encoding">
  ...
  <Callout type="note" title="Why Hexadecimal?">
    A byte can hold values from 0 to 255. We could write these in binary (8 digits per byte), decimal (1-3 digits per byte), or hexadecimal (always 2 digits per byte). Hex wins for representing raw byte streams because the width is consistent. When you see `C3 A9 E4 B8 AD`, you immediately know that's 5 bytes. Each pair of hex digits maps to exactly one byte, so boundaries are always obvious.
  </Callout>
...
<Step title="4. Complete Example">
  <Description>
    Let's trace through how **"Hi 👋"** gets converted to bytes. This is the exact process that happens before text reaches a language model. We'll show byte values in decimal here since that's what programming languages display by default.
  </Description>
  ...
  <div className="my-6 space-y-6">
    ...
    <div>
      ...
      <Description>
        Four characters became seven bytes. This sequence of integers is what text processing systems, including language models, actually receive as input. In Python, `list("Hi 👋".encode('utf-8'))` returns exactly this.
      </Description>
    </div>
  </div>
</Step>
