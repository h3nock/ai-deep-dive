...
<Step title="5. Implementation: The Embedding Matrix">
  ...
  <Description>
    When we first create this table, every cell is filled with random numbers. "King" might start at coordinates right next to "Sandwich." The map has no structure at all.
  </Description>

  <Description>
    Training is the process where the model discovers what each dimension should represent. Crucially, this happens **end-to-end** with the rest of the model. We don't create embeddings separately; they are just the first layer of the neural network.
  </Description>

  <Description>
    When the model tries to predict the next word and makes a mistake, the "correction signal" (gradients) flows backwards through the entire network, updating the transformer blocks and finally updating the embedding coordinates. The model learns the meaning of words *at the same time* it learns how to use them.
  </Description>
